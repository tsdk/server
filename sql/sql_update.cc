                 ...
                 ha_rows *found_return, ha_rows *updated_return)
{
  ...
  */
  ...
  /*
    ...
    if the following cases are not true:
    - There is no triggers
    - No binlog row logging
    ...
    - If we use a virtual not stored column in the WHERE clause
    - If we change a field used by a stored virtual column, which
      would require the column to be recalculated.

    Later we also ensure that we are only using one table (no sub queries)
  */
  if (!has_triggers && !binlog_is_row && !query_plan.using_filesort &&
      !query_plan.using_io_buffer &&
      !using_limit && !ignore &&
