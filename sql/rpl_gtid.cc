                              ...
                              char* errbuf)
{
  ...
  /*
    ...
    sequence number not lesser than that of the list.
    Exceptions of this inclusion rule are:
      A. the list may still refer to gtids from already deleted domains.
         Files containing them must have been purged, the file with the list
         is not yet.
    ...
    B and C may require user's attention so any inconsistency is
    diagnosed and *warned*.
  */
  for (l= 0, errbuf[0]= 0; l < glev->count; l++, errbuf[0]= 0)
  {
    ...
    if (!rb_state_gtid)
      sprintf(errbuf,
              ...
              "which is referred in Gtid list describing an earlier state; "
      ...
      sprintf(errbuf,
              ...
              "'%u-%u-%llu' of Gtid list describing an earlier state; "
              "the state might have been affected by smaller sequence number "
